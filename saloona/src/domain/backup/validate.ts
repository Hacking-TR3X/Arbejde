...
const WARNING_TEXT: Record<string, (n: number) => string> = {
  ...
  amountInvalid: (n) => `${plural(n, 'beløb', 'beløb')} var ugyldige og blev fjernet`,
  payInvalid: (n) => `${plural(n, 'betalingsmåde', 'betalingsmåder')} var ukendte og blev fjernet`,
  genderInvalid: (n) => `${plural(n, 'kunde', 'kunder')} havde et ukendt køn, som blev fjernet`,
  phoneInvalid: (n) => `${plural(n, 'telefonnummer', 'telefonnumre')} var ugyldige og blev fjernet`,
  priceInvalid: (n) => `${plural(n, 'standardpris', 'standardpriser')} var ugyldige og blev sprunget over`
};
...
export function readBackupText(text: string): ReadResult {
  ...
  const outClients = readClients(clients, source, w);
  const clientIds = new Set(outClients.map((c) => c.id));
  const outVisits = readVisits(visits, clientIds, w);
...
function readClients(list: unknown[], source: 'salonbog' | 'saloona', w: Warnings): ImportClient[] {
...
function readVisits(list: unknown[], clientIds: ReadonlySet<string>, w: Warnings): ImportVisit[] {
  ...
  for (const raw of list) {
    ...
    const id = own(raw, 'id');
    const clientId = own(raw, 'clientId');
    ...
    if (!isValidId(id) || !isValidId(clientId) || !treatment) {
      w.add('visitInvalid');
      continue;
    }
...
function readPrices(raw: unknown, w: Warnings): Map<string, number> {
  ...
  for (const key of Object.keys(raw)) {
    ...
    const ore = oreFromKroner(own(raw, key));
    if (!k || ore === null) {
      w.add('priceInvalid');
      continue;
    }
