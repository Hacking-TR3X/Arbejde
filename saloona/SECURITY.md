...
Seneste gennemgang: 2026-09-26. Første runde dækkede fase 1–4 og CI. Anden runde verificerede rettelserne i `dcf14d7` og `5a0ed60`. Tredje runde gennemgik `988c96c` (prisliste, migration 003, kalender): ingen nye permissions, netværkskald eller persondata i notifikationer. S-19–S-22 er rettet i commit'en efter. Fjerde runde gennemgik `8eb4d91` (bogstav-indeks, Barn/Uden køn, køn fra behandlinger, migration 004): ingen nye permissions, dependencies, netværkskald eller CSP-brud. Kun S-23 (Info) er nyt, og det er rettet i commit'en efter. Commit'en er godkendt med bemærkninger.
...
## Data og dataflow
Saloona gemmer kunders navn, køn, mærke, telefonnummer og note samt besøg med dato, behandling, beløb, betalingsmåde og note. Det er persondata efter GDPR. En note kan indeholde helbredsoplysninger, fx om allergi, så skriv kun det, der er nødvendigt. En kunde uden køn får automatisk et, når alle kundens behandlinger peger på det samme køn (prislistens valg, ellers navnet, fx "Herreklip"). Det sker kun, når der er noget nyt at gå efter, og et køn, som ejeren har fjernet, forbliver fjernet, se S-23.
...
- **WebView:** CSP i det byggede `index.html`: `default-src 'self'; script-src 'self'; style-src 'self'; img-src 'self' data:; font-src 'self'; connect-src 'self'; media-src 'none'; object-src 'none'; frame-src 'none'; worker-src 'none'; manifest-src 'none'; base-uri 'none'; form-action 'none'`.
  ...
  - Svelte's `style:`-direktiver (bogstav-indekset) sættes via CSSOM (`style.cssText`/`setProperty`), som `style-src 'self'` tillader. Der er ingen `style`-attributter i markup. Det er verificeret i Chromium med CSP'en: ingen `securitypolicyviolation`.
...
- **Import:**
  ...
  - Prislisten (`treatments`, kun Saloona-filer): et array med højst 2.000 punkter. Navnet renses og får højst 60 tegn, prisen skal være et tal på 0–1.000.000 kr., og varigheden skal være et heltal på 5–600 min. `gender` skal være `dame`, `herre` eller `alle` som egen property. Mangler feltet, læses det ud fra navnet. Andre værdier (andre typer, store bogstaver, mellemrum) giver en advarsel og læses også ud fra navnet. Ugyldige priser og varigheder fjernes, dubletter springes over, og nøglerne ligger kun i `Map`/arrays.
...
| S-23 | Info | Automatisk køn kan ikke fravælges. Hvis ejeren fjerner kønnet på en kunde med fx "Herreklip", sætter `fillGenders` det igen straks efter (`afterChange`) og ved hver start. Hvis en behandling på prislisten sættes til Dame/Herre, får alle berørte kunder uden køn det køn i én skrivning. Det sker uden Fortryd, og det rulles ikke tilbage, hvis valget ændres. Et køn, der allerede er sat, overskrives aldrig (`WHERE gender IS NULL`). Det er ikke et sikkerhedshul, men det strider mod dataminimering og korrekthed. Anbefaling: udfyld kun ved ny evidens (nyt besøg for kunden eller ændret køn på en behandling), kør start-udfyldningen én gang efter migration 004, og vis antal med Fortryd. | `src/lib/app.svelte.ts` `fillGenders`, `afterChange`, `start` | rettet: udfyldes kun ved start én gang (`gendersFilled`), efter et besøg (kun den kunde), efter import, og når en behandlings køn ændres (kun kunder med den behandling, med Fortryd). Et fjernet køn forbliver fjernet (e2e i `gender.spec.ts`). Regressionstests er med. |
